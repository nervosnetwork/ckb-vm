            ...
            match sew {
                8 => {
                    let offset = match $size {
                        8 => E8::get_unsafe($machine.element_ref(vs2, $size, j)).u64(),
                        16 => E16::get_unsafe($machine.element_ref(vs2, $size, j)).u64(),
                        32 => E32::get_unsafe($machine.element_ref(vs2, $size, j)).u64(),
                        64 => E64::get_unsafe($machine.element_ref(vs2, $size, j)).u64(),
                        _ => unreachable!(),
                    };
                    let data = $machine
                        .memory_mut()
...
macro_rules! v_vv_loop {
    ($inst:expr, $machine:expr, $body:expr) => {
        ...
        for j in 0..$machine.vl() as usize {
            ...
            match sew {
                8 => {
                    let b = E8::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E8::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                16 => {
                    let b = E16::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E16::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                32 => {
                    let b = E32::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E32::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                64 => {
                    let b = E64::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E64::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                128 => {
                    let b = E128::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E128::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                256 => {
                    let b = E256::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E256::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                512 => {
                    let b = E512::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E512::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                1024 => {
                    let b = E1024::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E1024::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                _ => unreachable!(),
...
macro_rules! w_wv_loop {
    ($inst:expr, $machine:expr, $body:expr, $sign:expr) => {
        ...
        for j in 0..$machine.vl() as usize {
            ...
            match sew {
                8 => {
                    let b = E16::get_unsafe($machine.element_ref(i.vs2(), sew * 2, j));
                    let a = if $sign != 0 {
                        E16::from(E8::get_unsafe($machine.element_ref(i.vs1(), sew, j))).lo_sext()
                    } else {
                        E16::from(E8::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                    };
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew * 2, j));
                }
                16 => {
                    let b = E32::get_unsafe($machine.element_ref(i.vs2(), sew * 2, j));
                    let a = if $sign != 0 {
                        E32::from(E16::get_unsafe($machine.element_ref(i.vs1(), sew, j))).lo_sext()
                    } else {
                        E32::from(E16::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                    };
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew * 2, j));
                }
                32 => {
                    let b = E64::get_unsafe($machine.element_ref(i.vs2(), sew * 2, j));
                    let a = if $sign != 0 {
                        E64::from(E32::get_unsafe($machine.element_ref(i.vs1(), sew, j))).lo_sext()
                    } else {
                        E64::from(E32::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                    };
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew * 2, j));
                }
                64 => {
                    let b = E128::get_unsafe($machine.element_ref(i.vs2(), sew * 2, j));
                    let a = if $sign != 0 {
                        E128::from(E64::get_unsafe($machine.element_ref(i.vs1(), sew, j))).lo_sext()
                    } else {
                        E128::from(E64::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                    };
                    let r = $body(b, a);
                    r.put($machine.element_mut(i.vd(), sew * 2, j));
                }
                128 => {
                    let b = E256::get_unsafe($machine.element_ref(i.vs2(), sew * 2, j));
                    let a = if $sign != 0 {
                        E256::from(E128::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                            .lo_sext()
                    } else {
                        E256::from(E128::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                    };
                ...
                256 => {
                    let b = E512::get_unsafe($machine.element_ref(i.vs2(), sew * 2, j));
                    let a = if $sign != 0 {
                        E512::from(E256::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                            .lo_sext()
                    } else {
                        E512::from(E256::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                    };
                ...
                512 => {
                    let b = E1024::get_unsafe($machine.element_ref(i.vs2(), sew * 2, j));
                    let a = if $sign != 0 {
                        E1024::from(E512::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                            .lo_sext()
                    } else {
                        E1024::from(E512::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                    };
                ...
                1024 => {
                    let b = E2048::get_unsafe($machine.element_ref(i.vs2(), sew * 2, j));
                    let a = if $sign != 0 {
                        E2048::from(E1024::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                            .lo_sext()
                    } else {
                        E2048::from(E1024::get_unsafe($machine.element_ref(i.vs1(), sew, j)))
                    };
        ...
        for j in 0..$machine.vl() as usize {
            ...
            match sew {
                8 => {
                    let b = E8::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E8::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E8::get_unsafe($machine.element_ref(i.vd(), sew, j));
                    let r = $body(b, a, c);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                16 => {
                    let b = E16::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E16::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E16::get_unsafe($machine.element_ref(i.vd(), sew, j));
                    let r = $body(b, a, c);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                32 => {
                    let b = E32::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E32::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E32::get_unsafe($machine.element_ref(i.vd(), sew, j));
                    let r = $body(b, a, c);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                64 => {
                    let b = E64::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E64::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E64::get_unsafe($machine.element_ref(i.vd(), sew, j));
                    let r = $body(b, a, c);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                128 => {
                    let b = E128::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E128::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E128::get_unsafe($machine.element_ref(i.vd(), sew, j));
                    let r = $body(b, a, c);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                256 => {
                    let b = E256::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E256::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E256::get_unsafe($machine.element_ref(i.vd(), sew, j));
                    let r = $body(b, a, c);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                512 => {
                    let b = E512::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E512::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E512::get_unsafe($machine.element_ref(i.vd(), sew, j));
                    let r = $body(b, a, c);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                1024 => {
                    let b = E1024::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E1024::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E1024::get_unsafe($machine.element_ref(i.vd(), sew, j));
                    let r = $body(b, a, c);
                    r.put($machine.element_mut(i.vd(), sew, j));
                }
                _ => unreachable!(),
        ...
        for j in 0..$machine.vl() as usize {
            ...
            match sew {
                8 => {
                    let b = E8::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E8::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E8::get_unsafe($machine.element_ref(i.vd(), sew, j * 2));
                    let d = E8::get_unsafe($machine.element_ref(i.vd(), sew, j * 2 + 1));
                    let r = $body(b, a, c, d);
                ...
                16 => {
                    let b = E16::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E16::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E16::get_unsafe($machine.element_ref(i.vd(), sew, j * 2));
                    let d = E16::get_unsafe($machine.element_ref(i.vd(), sew, j * 2 + 1));
                    let r = $body(b, a, c, d);
                    r.0.put($machine.element_mut(i.vd(), sew, j * 2));
                    r.1.put($machine.element_mut(i.vd(), sew, j * 2 + 1));
                }
                32 => {
                    let b = E32::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E32::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E32::get_unsafe($machine.element_ref(i.vd(), sew, j * 2));
                    let d = E32::get_unsafe($machine.element_ref(i.vd(), sew, j * 2 + 1));
                    let r = $body(b, a, c, d);
                    r.0.put($machine.element_mut(i.vd(), sew, j * 2));
                    r.1.put($machine.element_mut(i.vd(), sew, j * 2 + 1));
                }
                64 => {
                    let b = E64::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E64::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E64::get_unsafe($machine.element_ref(i.vd(), sew, j * 2));
                    let d = E64::get_unsafe($machine.element_ref(i.vd(), sew, j * 2 + 1));
                    let r = $body(b, a, c, d);
                    r.0.put($machine.element_mut(i.vd(), sew, j * 2));
                    r.1.put($machine.element_mut(i.vd(), sew, j * 2 + 1));
                }
                128 => {
                    let b = E128::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E128::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E128::get_unsafe($machine.element_ref(i.vd(), sew, j * 2));
                    let d = E128::get_unsafe($machine.element_ref(i.vd(), sew, j * 2 + 1));
                    let r = $body(b, a, c, d);
                    r.0.put($machine.element_mut(i.vd(), sew, j * 2));
                    r.1.put($machine.element_mut(i.vd(), sew, j * 2 + 1));
                }
                256 => {
                    let b = E256::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E256::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E256::get_unsafe($machine.element_ref(i.vd(), sew, j * 2));
                    let d = E256::get_unsafe($machine.element_ref(i.vd(), sew, j * 2 + 1));
                    let r = $body(b, a, c, d);
                    r.0.put($machine.element_mut(i.vd(), sew, j * 2));
                    r.1.put($machine.element_mut(i.vd(), sew, j * 2 + 1));
                }
                512 => {
                    let b = E512::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E512::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E512::get_unsafe($machine.element_ref(i.vd(), sew, j * 2));
                    let d = E512::get_unsafe($machine.element_ref(i.vd(), sew, j * 2 + 1));
                    let r = $body(b, a, c, d);
                    r.0.put($machine.element_mut(i.vd(), sew, j * 2));
                    r.1.put($machine.element_mut(i.vd(), sew, j * 2 + 1));
                }
                1024 => {
                    let b = E1024::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                    let a = E1024::get_unsafe($machine.element_ref(i.vs1(), sew, j));
                    let c = E1024::get_unsafe($machine.element_ref(i.vd(), sew, j * 2));
                    let d = E1024::get_unsafe($machine.element_ref(i.vd(), sew, j * 2 + 1));
                    let r = $body(b, a, c, d);
                    r.0.put($machine.element_mut(i.vd(), sew, j * 2));
                    r.1.put($machine.element_mut(i.vd(), sew, j * 2 + 1));
                }
...
macro_rules! v_vs_loop {
    ($inst:expr, $machine:expr, $body:expr) => {
        ...
        if $machine.vl() != 0 {
            match sew {
                8 => {
                    let mut ret = E8::get_unsafe($machine.element_ref(i.vs1(), sew, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = E8::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                        ret = $body(ret, vs2);
                    }
                    ret.put($machine.element_mut(i.vd(), sew, 0));
                }
                16 => {
                    let mut ret = E16::get_unsafe($machine.element_ref(i.vs1(), sew, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = E16::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                        ret = $body(ret, vs2);
                    }
                    ret.put($machine.element_mut(i.vd(), sew, 0));
                }
                32 => {
                    let mut ret = E32::get_unsafe($machine.element_ref(i.vs1(), sew, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = E32::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                        ret = $body(ret, vs2);
                    }
                    ret.put($machine.element_mut(i.vd(), sew, 0));
                }
                64 => {
                    let mut ret = E64::get_unsafe($machine.element_ref(i.vs1(), sew, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = E64::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                        ret = $body(ret, vs2);
                    }
                    ret.put($machine.element_mut(i.vd(), sew, 0));
                }
                128 => {
                    let mut ret = E128::get_unsafe($machine.element_ref(i.vs1(), sew, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = E128::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                        ret = $body(ret, vs2);
                    }
                    ret.put($machine.element_mut(i.vd(), sew, 0));
                }
                256 => {
                    let mut ret = E256::get_unsafe($machine.element_ref(i.vs1(), sew, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = E256::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                        ret = $body(ret, vs2);
                    }
                    ret.put($machine.element_mut(i.vd(), sew, 0));
                }
                512 => {
                    let mut ret = E512::get_unsafe($machine.element_ref(i.vs1(), sew, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = E512::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                        ret = $body(ret, vs2);
                    }
                    ret.put($machine.element_mut(i.vd(), sew, 0));
                }
                1024 => {
                    let mut ret = E1024::get_unsafe($machine.element_ref(i.vs1(), sew, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = E1024::get_unsafe($machine.element_ref(i.vs2(), sew, j));
                        ret = $body(ret, vs2);
                    }
                    ret.put($machine.element_mut(i.vd(), sew, 0));
                }
...
macro_rules! w_vs_loop {
    ($inst:expr, $machine:expr, $body:expr, $sign:expr) => {
        ...
        if $machine.vl() != 0 {
            match sew {
                8 => {
                    let mut ret = E16::get_unsafe($machine.element_ref(i.vs1(), sew * 2, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = if $sign != 0 {
                            E16::from(E8::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                                .lo_sext()
                        } else {
                            E16::from(E8::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                        };
                ...
                16 => {
                    let mut ret = E32::get_unsafe($machine.element_ref(i.vs1(), sew * 2, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = if $sign != 0 {
                            E32::from(E16::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                                .lo_sext()
                        } else {
                            E32::from(E16::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                        };
                ...
                32 => {
                    let mut ret = E64::get_unsafe($machine.element_ref(i.vs1(), sew * 2, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = if $sign != 0 {
                            E64::from(E32::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                                .lo_sext()
                        } else {
                            E64::from(E32::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                        };
                ...
                64 => {
                    let mut ret = E128::get_unsafe($machine.element_ref(i.vs1(), sew * 2, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = if $sign != 0 {
                            E128::from(E64::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                                .lo_sext()
                        } else {
                            E128::from(E64::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                        };
                ...
                128 => {
                    let mut ret = E256::get_unsafe($machine.element_ref(i.vs1(), sew * 2, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = if $sign != 0 {
                            E256::from(E128::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                                .lo_sext()
                        } else {
                            E256::from(E128::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                        };
                ...
                256 => {
                    let mut ret = E512::get_unsafe($machine.element_ref(i.vs1(), sew * 2, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = if $sign != 0 {
                            E512::from(E256::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                                .lo_sext()
                        } else {
                            E512::from(E256::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                        };
                ...
                512 => {
                    let mut ret = E1024::get_unsafe($machine.element_ref(i.vs1(), sew * 2, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = if $sign != 0 {
                            E1024::from(E512::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                                .lo_sext()
                        } else {
                            E1024::from(E512::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                        };
                ...
                1024 => {
                    let mut ret = E2048::get_unsafe($machine.element_ref(i.vs1(), sew * 2, 0));
                    for j in 0..$machine.vl() as usize {
                        if i.vm() == 0 && !$machine.get_bit(0, j) {
                            continue;
                        }
                        let vs2 = if $sign != 0 {
                            E2048::from(E1024::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                                .lo_sext()
                        } else {
                            E2048::from(E1024::get_unsafe($machine.element_ref(i.vs2(), sew, j)))
                        };
...
macro_rules! x_m_loop {
    ($inst:expr, $machine:expr, $body:expr) => {
        require_vill!($machine);
        let i = VVtype($inst);
        let vs2 = E2048::get_unsafe($machine.element_ref(i.vs2(), VLEN as u64, 0));
        let m = if i.vm() == 0 {
            E2048::get_unsafe($machine.element_ref(0, VLEN as u64, 0))
        } else {
            E2048::MAX_U
        };
        let vl = $machine.vl();
...
macro_rules! m_m_loop {
    ($inst:expr, $machine:expr, $body:expr) => {
        ...
        require!(i.vd() != i.vs2(), String::from("require: vd != vs2"));
        let vs2 = E2048::get_unsafe($machine.element_ref(i.vs2(), VLEN as u64, 0));
        let vd = E2048::get_unsafe($machine.element_ref(i.vd(), VLEN as u64, 0));
        let m = if i.vm() == 0 {
            E2048::get_unsafe($machine.element_ref(0, VLEN as u64, 0))
        } else {
            E2048::MAX_U
