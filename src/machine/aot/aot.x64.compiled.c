...
static const unsigned char bf_actions[1396] = {
  ...
  2,72,199,194,237,94,195,248,3,72,199,194,237,94,195,255,248,11,87,86,82,72,
  137,199,72,193,231,235,72,141,183,233,72,1,252,247,72,49,252,246,72,199,194,
  237,232,244,12,90,94,95,195,255,248,13,86,72,137,214,72,137,193,72,193,252,
  ...
  11,88,72,137,193,72,1,252,241,72,131,252,233,1,72,193,252,233,235,72,129,
  252,249,239,15,131,244,248,72,141,151,233,15,182,20,10,131,252,250,1,15,132,
  244,247,72,141,151,233,255,198,4,10,1,80,72,137,200,232,244,11,88,252,233,
  244,247,248,1,72,199,194,0,0,0,0,94,195,248,2,72,199,194,237,94,195,255,248,
  14,72,137,183,233,76,137,135,233,76,137,143,233,93,91,65,95,65,94,65,93,65,
  ...
  63,41,209,72,211,224,137,209,255,72,199,192,237,252,233,244,14,255,72,1,135,
  233,72,139,135,233,72,57,135,233,15,134,244,247,255,72,199,135,233,237,252,
  233,245,255,72,137,135,253,240,131,233,255,72,199,194,237,232,244,13,72,131,
  252,250,0,15,133,244,247,72,199,194,237,232,244,10,72,131,252,250,0,15,133,
  244,247,72,141,151,233,255,136,12,2,255,102,137,12,2,255,72,137,12,2,255,
  252,233,244,248,248,1,72,137,208,252,233,244,14,248,2,255,72,199,194,237,
  232,244,13,72,131,252,250,0,15,133,244,247,72,137,194,72,129,194,239,15,130,
  244,247,72,129,252,250,239,15,135,244,247,72,141,151,233,255,15,182,12,2,
...
#line 402 "src/machine/aot/aot.x64.c"
  ...
  //| pop rsi
  //| ret
  dasm_put(Dst, 299, CKB_VM_ASM_RET_OUT_OF_BOUND);
#line 415 "src/machine/aot/aot.x64.c"
  ...
  //| pop r12
  //| ret
  dasm_put(Dst, 335, Dt1(->registers[REGISTER_RA]), Dt1(->registers[REGISTER_SP]), Dt1(->registers[REGISTER_A0]));
...
int aot_memory_write(AotContext* context, AotValue address, AotValue v, uint32_t size)
{
  ...
  dasm_put(Dst, 1219, size, size, Dt1(->memory));
...
int aot_memory_read(AotContext* context, uint32_t target, AotValue address, uint32_t size)
{
  ...
  //| ja >1
  //| lea rdx, machine->memory
  dasm_put(Dst, 1286, size, size, CKB_VM_ASM_RISCV_MAX_MEMORY, Dt1(->memory));
...
static int aot_mov_internal(AotContext* context, riscv_register_t target, AotValue value, x64_register_t x64_temp_reg)
{
  ...
  switch (value.tag) {
    case AOT_TAG_REGISTER:
      ...
      if (VALID_X64_REGISTER(loc1) && VALID_X64_REGISTER(loc2)) {
      dasm_put(Dst, 1346, (loc2), (loc1));
      } else if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1354, (loc1), Dt1(->registers[value.value.reg]));
      } else if (VALID_X64_REGISTER(loc2)) {
      dasm_put(Dst, 1211, (loc2), Dt1(->registers[target]));
      } else {
      dasm_put(Dst, 1362, (x64_temp_reg), Dt1(->registers[value.value.reg]), (x64_temp_reg), Dt1(->registers[target]));
      }
#line 1387 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_IMMEDIATE:
      ...
      if (value.value.i > 0xFFFFFFFF && ((value.value.i & 0xFFFFFFFF80000000) != 0xFFFFFFFF80000000)) {
        loc1 = riscv_reg_to_x64_reg(target);
      dasm_put(Dst, 1377, (x64_temp_reg), value.value.i >> 32, (x64_temp_reg), (x64_temp_reg), value.value.i & 0xFFFFFFFF);
        if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1346, (x64_temp_reg), (loc1));
        } else {
...
#line 1390 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      loc1 = riscv_reg_to_x64_reg(target);
      if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1346, (value.value.x64_reg), (loc1));
      } else {
...
static int aot_mov_x64(AotContext* context, x64_register_t x64_target, AotValue value)
{
  ...
  switch (value.tag) {
    case AOT_TAG_REGISTER:
      ...
      if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1346, (loc1), (x64_target));
      } else {
      dasm_put(Dst, 1354, (x64_target), Dt1(->registers[value.value.reg]));
      }
#line 1406 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_IMMEDIATE:
      ...
      if (value.value.i > 0xFFFFFFFF && ((value.value.i & 0xFFFFFFFF80000000) != 0xFFFFFFFF80000000)) {
      dasm_put(Dst, 1377, (x64_target), value.value.i >> 32, (x64_target), (x64_target), value.value.i & 0xFFFFFFFF);
      } else {
...
#line 1409 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      dasm_put(Dst, 1346, (value.value.x64_reg), (x64_target));
