...
const SC_ONLY_DATA_ADDR: u64 = 0x11000;
...
pub fn test_sc_only() {
    ...
    assert!(ret.is_ok());
    assert_eq!(ret.unwrap(), 0);
    assert_eq!(machine.machine.memory_mut().load64(&SC_ONLY_DATA_ADDR).unwrap(), 4);
    ...
    #[cfg(has_asm)]
    {
        ...
        assert!(ret_asm.is_ok());
        assert_eq!(ret_asm.unwrap(), 0);
        assert_eq!(machine_asm.machine.memory_mut().load64(&SC_ONLY_DATA_ADDR).unwrap(), 4);
    }
}
